...
import { Mountain, TreePine, Eye } from "lucide-react";

interface TerrainContextCardProps {
  profile: GeospatialContextProfile;
}

export default function TerrainContextCard({ profile }: TerrainContextCardProps) {
  const elements = [
    { label: "Terrain Geomorphology", value: profile.terrain_type, icon: Mountain },
    { label: "Environment Nature", value: profile.environment_type, icon: TreePine },
    { label: "Dominant Landscape", value: profile.dominant_landscape, icon: Eye },
  ];
