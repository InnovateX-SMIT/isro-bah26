...
export default function GeospatialMap({ context, loading, error }: GeospatialMapProps) {
  ...
  // Update map features when context changes
  useEffect(() => {
    ...
    try {
      ...
      const bounds: [number, number, number, number] = [
        context.bounds.west,
        context.bounds.south,
        context.bounds.east,
        context.bounds.north
      ];
      ...
      const popup = new maplibregl.Popup({ 
        ...
        <div style="font-family: monospace; font-size: 10px; color: #e2e8f0; background: #070a13; border: 1px solid #1e293b; padding: 8px; border-radius: 4px; box-shadow: 0 0 10px rgba(6,182,212,0.3)">
          ...
          <div>CRS: ${context.crs || "N/A"}</div>
          <div>EPSG: ${context.epsg || "N/A"}</div>
          <div>LAT: ${context.center.lat.toFixed(6)}</div>
          <div>LON: ${context.center.lon.toFixed(6)}</div>
        </div>
      `);

      const marker = new maplibregl.Marker({ element: el })
        .setLngLat([context.center.lon, context.center.lat])
        .setPopup(popup)
        .addTo(map);
    ...
    <div className="relative w-full h-full border border-border bg-card/25 overflow-hidden">
      ...
      {context && (
        <div className="absolute bottom-4 left-4 z-10 bg-card/90 border border-border/80 px-3 py-2 font-mono text-[9px] text-muted-foreground space-y-1 backdrop-blur-sm pointer-events-none">
          ...
          <div><span className="text-slate-300">LAT:</span> {context.center.lat.toFixed(6)} N</div>
          <div><span className="text-slate-300">LON:</span> {context.center.lon.toFixed(6)} E</div>
        </div>
      )}
    </div>
  );
